...
{
	ILOG_DEBUG
	    ("dummy_identify: network_type: %s, network_name: %s, network_id: %s\n",
	...
	gchar *name =
	    g_strconcat(network_name, " (", DUMMY_PROVIDER_NAME, ") ", NULL);
	...
	if (g_strcmp0(DUMMY_PROVIDER_NAME, gconf_service_type) == 0) {
		...
		identify_cb(ICD_SRV_IDENTIFIED, DUMMY_PROVIDER_TYPE,	/* service type */
			    name, 0,	/* XXX: service attributes */
			    DUMMY_PROVIDER_ID, 0,	/* XXX: service priority */
			    network_type,
			    network_attrs, network_id, identify_cb_token);

	} else {
		ILOG_DEBUG("dummy_identify: NO MATCH\n");
		identify_cb(ICD_SRV_UNKNOWN, DUMMY_PROVIDER_TYPE,	/* service type */
			    name, 0,	/* XXX: service attributes */
			    DUMMY_PROVIDER_ID, 0,	/* XXX: service priority */
			    network_type,
			    network_attrs, network_id, identify_cb_token);
